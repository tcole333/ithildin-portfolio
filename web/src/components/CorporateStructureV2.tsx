...
import { graphStratify, sugiyama, layeringSimplex, coordSimplex, decrossTwoLayer } from 'd3-dag';
...
export default function CorporateStructureV2({ data, height = 600 }: Props) {
  ...
  const [collapsedSet, setCollapsedSet] = useState<Set<string>>(() => {
    ...
    data.nodes.forEach(n => {
      if ((n as any).collapsed) initial.add(n.id);
    });
    return initial;
  });
  ...
  useEffect(() => {
    ...
    try {
      ...
      setLayoutError(`graphStratify failed: ${err.message || String(err)}`);
      return;
    }
    ...
    const layout = sugiyama()
      .nodeSize((node: any) => {
        ...
        return [nodeLayoutHeight(d) + 12, nodeWidth(d) + 40];
      })
      .gap([16, 40])
      .layering(layeringSimplex())
      .decross(decrossTwoLayer()) // Use two layer decrossing for large dense graphs to avoid slow/hanging layout computations
      .coord(coordSimplex());

    let layoutW: number, layoutH: number;
    try {
      const result = layout(dag);
      layoutW = result.height; // Swapped
      layoutH = result.width;  // Swapped
    } catch (err: any) {
      console.error('[CorporateStructureV2] sugiyama layout failed:', err);
      setLayoutError(`sugiyama layout failed: ${err.message || String(err)}`);
      return;
    }
    ...
    for (const dagNode of dag.nodes()) {
      posMap.set(dagNode.data.id, { x: dagNode.y!, y: dagNode.x! });
    }
    ...
    // Scale so layout width fills container, but don't shrink below 0.6x to maintain readability
    const fitScale = Math.max(0.6, Math.min(1, containerWidth / totalW));
    const svgH = Math.max(300, Math.min(height, Math.ceil(totalH * fitScale) + 40));
    ...
    for (const edge of edgesForRender) {
      ...
      const srcW = nodeWidth(srcNode) / 2;
      const tgtW = nodeWidth(tgtNode) / 2;

      const rightward = src.x < tgt.x;
      const y1 = src.y + pad + edge.offset;
      const y2 = tgt.y + pad + edge.offset;
      const x1 = src.x + (rightward ? srcW : -srcW) + pad;
      const x2 = tgt.x + (rightward ? -tgtW : tgtW) + pad;
      ...
      if (rightward) {
        if (Math.abs(y2 - y1) < r) {
           ...
           pathD = `M${x1},${y1} H${midX - r} Q${midX},${y1} ${midX},${y1 + r * dirY} V${y2 - r * dirY} Q${midX},${y2} ${midX + r},${y2} H${x2}`;
        }
      } else {
        pathD = `M${x1},${y1} H${x1 + 16} V${y1 - 24} H${x2 - 16} V${y2} H${x2}`;
      }
      ...
      if (labelText && (Math.abs(x1 - x2) > 24 || Math.abs(y1 - y2) > 14)) {
    ...
    for (const dagNode of dag.nodes()) {
      ...
      // Use posMap since we swapped x and y there
  ...
  return (
    <div ref={containerRef} className="surface p-5 relative">
      {layoutError && (
        <div className="absolute inset-0 bg-[rgba(18,21,27,0.9)] z-50 flex items-center justify-center p-6 text-center">
          <div className="bg-[rgba(207,102,121,0.1)] border border-[color:var(--color-red)] rounded p-4 max-w-xl">
            <h4 className="text-[color:var(--color-red)] font-semibold mb-2">Visualization Layout Error</h4>
            <p className="text-mithril text-sm font-mono whitespace-pre-wrap text-left">{layoutError}</p>
          </div>
        </div>
      )}
      <div className="mb-4">
